// 请求代码
(function() {
    // 发起请求
    function request(option) {
        if (!option)
            throw '未填写请求参数';

        let xhr = new XMLHttpRequest();
        xhr.open(option.method || 'GET', option.url);

        if (option.send == 0)
            xhr.send(0);
        else
            xhr.send(option.send || null);

        if (option.result) {
            xhr.onload = function() {
                if (!option.resType || option.resType == 'text')
                    option.result(this.responseText, this.status);
                else if (option.resType == 'json')
                    option.result(JSON.parse(this.response), this.status);
                // 如果需要其他的返回类型
                // 可以加
            }
        }
    }

    // 将时间字符串转化为layui的时间格式
    function str2layuiDate(str) {
        let date = new Date(str);
        return {
            year: date.getFullYear(),
            month: date.getMonth() + 1,
            date: date.getDate(),
            hours: date.getHours(),
            minutes: date.getMinutes(),
            seconds: date.getSeconds()
        }
    }

    // 判断时间是否处于某时间内
    function betweenTime(date, start, end) {
        if (date.getTime() < start.getTime() || date.getTime() > end.getTime())
            return false;
        return true;
    }

    let messageBody = document.getElementById('message-body'); // 存放数据的表格内容
    let timeDate = document.getElementById('time-date'); // 存放筛选的时间
    let heightStart = document.getElementById('height-start'); // 存放筛选的最低高程
    let heightEnd = document.getElementById('height-end'); // 存放筛选的最高高程
    let xStart = document.getElementById('x-start'); // 存放筛选的最底经度
    let xEnd = document.getElementById('x-end'); // 存放筛选的最高经度
    let yStart = document.getElementById('y-start'); // 存放筛选的最底纬度
    let yEnd = document.getElementById('y-end'); // 存放筛选的最高纬度
    let showBtn = document.getElementById('show-btn'); // 显示按钮
    let clearBtn = document.getElementById('clear-btn'); // 清除按钮

    // 存放数据
    let datas = {
        tableData: [], // 表数据
        showTableData: [], // 临时存储筛选后数据
        screenTime: {
            start: '',
            end: ''
        }, //  筛选的时间
        screenHeight: {
            start: '',
            end: ''
        }, // 筛选的高程
        screenX: {
            start: '',
            end: ''
        },
        screenY: {
            start: '',
            end: ''
        },
    };

    // 显示表格
    function showTable() {
        messageBody.innerHTML = '';
        for (let i = 0; i < datas.showTableData.length; i++) {
            let d = datas.showTableData[i];
            messageBody.innerHTML += '<tr>' +
                '<td>' + d.photo_name + '</td>' +
                '<td>' + d.time + '</td>' +
                '<td>' + d.row_x + '</td>' +
                '<td>' + d.row_y + '</td>' +
                '<td>' + d.row_z + '</td>' +
                '</tr>';
        }
    }

    //================================================================================
    // 筛选方法
    //================================================================================

    function screeningStart() {
        // 清除数组
        datas.showTableData.splice(0, datas.showTableData.length);
    }

    // 筛选
    // 全部筛选写在一起，效率比较高
    function screening() {
        for (let i = 0; i < datas.tableData.length; i++) {
            let d = datas.tableData[i];
            // 时间段
            if (datas.screenTime.start != '' && datas.screenTime.end != '')
                if (!betweenTime(new Date(d.time), datas.screenTime.start, datas.screenTime.end))
                    continue; // 如果不在时间区间内

                // 高程
            if (datas.screenHeight.start != '' && datas.screenHeight.end != '')
                if (parseFloat(d.row_z) < parseFloat(datas.screenHeight.start) || parseFloat(d.row_z) > parseFloat(datas.screenHeight.end))
                    continue;

                // 经度
            if (datas.screenX.start != '' && datas.screenX.end != '')
                if (parseFloat(d.row_x.substring(1)) < parseFloat(datas.screenX.start) || parseFloat(d.row_x.substring(1)) > parseFloat(datas.screenX.end))
                    continue;

                // 纬度
            if (datas.screenY.start != '' && datas.screenY.end != '')
                if (parseFloat(d.row_y.substring(1)) < parseFloat(datas.screenY.start) || parseFloat(d.row_y.substring(1)) > parseFloat(datas.screenY.end))
                    continue;

                // 其他筛选方法

            datas.showTableData.push(d);
        }
    }
    //=================================================================================


    //================================================================================
    // Layui处理
    //================================================================================
    layui.use('laydate', function() {
        let laydate = layui.laydate;

        // 时间选择
        laydate.render({
            elem: '#time-date',
            range: true,
            type: 'datetime',
            done: function(value) {
                if (value != '') {
                    datas.screenTime.start = new Date(value.substring(0, 19));
                    datas.screenTime.end = new Date(value.substring(22));
                } else {
                    datas.screenTime.start = '';
                    datas.screenTime.end = '';
                }
            }
        });
    });
    //=================================================================================

    //================================================================================
    // 控件交互
    //================================================================================
    showBtn.addEventListener('click', function() {
        screeningStart();
        screening();
        showTable();
    });

    clearBtn.addEventListener('click', function() {
        timeDate.value = '';
        heightStart.value = '';
        heightEnd.value = '';
        xStart.value = '';
        xEnd.value = '';
        yStart.value = '';
        yEnd.value = '';
        datas.screenHeight.start = '';
        datas.screenHeight.end = '';
        datas.screenX.start = '';
        datas.screenX.end = '';
        datas.screenY.start = '';
        datas.screenY.end = '';
    });

    // 绑定高程
    heightStart.addEventListener('input', function() {
        datas.screenHeight.start = this.value;
    });
    heightEnd.addEventListener('input', function() {
        datas.screenHeight.end = this.value;
    });
    xStart.addEventListener('input', function() {
        datas.screenX.start = this.value;
    });
    xEnd.addEventListener('input', function() {
        datas.screenX.end = this.value;
    });
    yStart.addEventListener('input', function() {
        datas.screenY.start = this.value;
    });
    yEnd.addEventListener('input', function() {
        datas.screenY.end = this.value;
    });
    //=================================================================================

    request({
        url: '../../getOriginalData',
        method: 'POST',
        resType: 'json',
        result: function(res, status) {
            datas.tableData = res;
            datas.showTableData = res.slice();
            // 渲染
            // TODO 对null的处理
            showTable();
        }
    });

    // 当前版本说明

    layui.use('layer', function() {
        let layer = layui.layer;

        layer.open({
            type: 1,
            content: '<div style="padding: 10px">筛选方法都已实现，由于数据不全并且要求有点模棱两可，因此筛选条件不一定正确，有错误的地方我会积极修改。</div>'
        });
    });
}());