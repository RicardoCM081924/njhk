...
//        }
    ...
    @LogAnnotation(operateType = "工程概览查询")
    @ResponseBody
    @RequestMapping("getRollingLayMessage")
    public ArrayList<String> getRollingLayMessage() {
        ArrayList<String> list = new ArrayList<>();
        //查询当前共碾压了多少层
        List<Integer> layList = homePageService.getLayList();

        //对每一层碾压情况进行统计
        for (Integer layNum : layList) {
            StringBuilder sb = new StringBuilder("第"+nums[layNum-1]+"层级于");

            //碾压完成时间
            String time = homePageService.getFinishTime(layNum).substring(0,10);

            //最大级配等级
            String maxGrading = homePageService.getOneLayerMaxGrading(layNum);

            //查询密度预警条件
            Long densityWarning = warningService.getDensityWarning();

            //密度偏低xxx处
            String unqualifiedNum = homePageService.getUnqualifiedNum(layNum,densityWarning);

            //密度合格xxx处
            String qualifiedNum = homePageService.getQualifiedNum(layNum,densityWarning);

            //最小密度
            String minDensity = homePageService.getOneLayerMinDensity(layNum);

            //最大密度
            String maxDensity = homePageService.getOneLayerMaxDensity(layNum);

            list.add(sb.append(time).append("碾压完成，最大级配等级为").append(maxGrading).append("，密度偏低").append(unqualifiedNum)
                    .append("处，密度合格").append(unqualifiedNum).append("处，最小密度为").append(minDensity).append("kg/m³，最大密度为")
                    .append(maxDensity).append("kg/m³。").toString());
        }
        return list;
    }
